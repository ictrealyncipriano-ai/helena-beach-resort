        ...
        <div class="reveal">
            <form method="POST" action="{{ route('booking.portal.lookup') }}" class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm p-8">
                ...
                <div class="mb-5">
                    <label for="email" class="block text-sm font-medium text-gray-700 dark:text-slate-200 mb-1">
                        <x-icons name="email" class="w-3.5 h-3.5 inline -mt-0.5 mr-1 text-gray-400 dark:text-slate-500" />
                        Email Address
                    </label>
                    <input type="email" name="email" id="email" required
                        value="{{ old('email') }}"
                        class="w-full px-4 py-2.5 border border-gray-200 dark:bg-slate-800 dark:border-slate-600 dark:text-white dark:placeholder-slate-400 rounded-xl text-sm focus:ring-2 focus:ring-teal-500 focus:border-teal-500 dark:focus:border-teal-500 dark:ring-teal-500/20 outline-none transition-all"
                        placeholder="you@example.com">
                </div>

                <div class="mb-6">
                    <label for="reference_code" class="block text-sm font-medium text-gray-700 dark:text-slate-200 mb-1">
                        <x-icons name="tag" class="w-3.5 h-3.5 inline -mt-0.5 mr-1 text-gray-400 dark:text-slate-500" />
                        Reference Code
                    </label>
                    <input type="text" name="reference_code" id="reference_code" required
                        value="{{ old('reference_code') }}"
                        class="w-full px-4 py-2.5 border border-gray-200 dark:bg-slate-800 dark:border-slate-600 dark:text-white dark:placeholder-slate-400 rounded-xl text-sm focus:ring-2 focus:ring-teal-500 focus:border-teal-500 dark:focus:border-teal-500 dark:ring-teal-500/20 outline-none font-mono transition-all"
            ...
            <div class="text-center mt-6">
                <p class="text-sm text-gray-500 dark:text-slate-400">
                    ...
                    <a href="{{ route('book') }}" class="text-teal-600 dark:text-teal-300 hover:text-teal-700 dark:hover:text-teal-300 font-medium">Book Now</a>
                </p>
            </div>
        </div>
    </div>
