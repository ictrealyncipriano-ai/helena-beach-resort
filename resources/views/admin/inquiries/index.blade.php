...
<div x-data="inquiryModal()" class="space-y-6">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-5 py-3 text-right">
                                <div class="flex items-center justify-end gap-1">
                                    <button type="button" @@click="openView({{ $inquiry->id }})" class="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="View">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                                    </button>
                                    <button type="button" @@click="openEdit({{ $inquiry->id }})" class="p-1.5 text-gray-400 hover:text-teal-600 hover:bg-teal-50 rounded-lg transition-colors" title="Edit">
    ...
    <x-admin.modal name="inquiry-view" size="xl">
        ...
        <div class="flex flex-wrap items-center justify-end gap-2 pt-5 mt-6 border-t border-gray-100">
...
window.inquiryModal = function() {
    return {
        ...
        statusBadgeClass(status) {
            return status === 'confirmed' ? 'bg-emerald-50 text-emerald-700 ring-emerald-600/20'
                : status === 'cancelled' ? 'bg-red-50 text-red-700 ring-red-600/20'
                : status === 'expired' ? 'bg-gray-50 text-gray-600 ring-gray-500/20'
                : 'bg-amber-50 text-amber-700 ring-amber-600/20';
        },

        typeBadgeClass(type) {
            return type === 'day_tour' ? 'bg-sky-50 text-sky-700 ring-sky-600/20'
                : type === 'overnight' ? 'bg-amber-50 text-amber-700 ring-amber-600/20'
                : 'bg-gray-50 text-gray-600 ring-gray-500/20';
        },

        paymentBadgeClass(key) {
            return key === 'paid' ? 'bg-teal-50 text-teal-700 ring-teal-600/20'
                : key === 'refunded' || key === 'failed' ? 'bg-red-50 text-red-700 ring-red-600/20'
                : 'bg-gray-50 text-gray-600 ring-gray-500/20';
        },
