...
@section('breadcrumb')
    <x-admin.breadcrumb :items="[['label' => 'Inquiries', 'url' => route('admin.inquiries.index')], ['label' => $inquiry->reference_code, 'url' => route('admin.inquiries.show', $inquiry)], ['label' => 'Edit']]" />
@endsection
...
<div class="max-w-3xl">
    <form method="POST" action="{{ route('admin.inquiries.update', $inquiry) }}" class="space-y-6">
        ...
        <div class="flex items-center justify-end gap-3">
            ...
            <button type="submit" class="px-6 py-2 text-sm font-medium text-white bg-teal-700 rounded-lg hover:bg-teal-800 transition-colors shadow-sm">Update Inquiry</button>
        </div>
    </form>
</div>
@endsection