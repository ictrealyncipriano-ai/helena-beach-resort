...
@section('breadcrumb')
    <nav class="flex items-center gap-1 text-xs text-gray-500">
        ...
        <span>/</span>
        <a href="{{ route('admin.services.index') }}" class="hover:text-teal-600 transition-colors">Services</a>
        <span>/</span>
        <span class="text-gray-700 font-medium">{{ $service->exists ? $service->name : 'New' }}</span>
    </nav>
@endsection
...
<div class="max-w-2xl">
    <form method="POST" action="{{ $service->exists ? route('admin.services.update', $service) : route('admin.services.store') }}" class="space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 space-y-5">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $service->name) }}" required maxlength="255" class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-300 focus:border-teal-400">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Icon</label>
                    <input type="text" name="icon" value="{{ old('icon', $service->icon) }}" maxlength="50" placeholder="e.g. wifi, pool, parking" class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-300 focus:border-teal-400">
                </div>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Description</label>
                <textarea name="description" rows="4" class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-300 focus:border-teal-400">{{ old('description', $service->description) }}</textarea>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                ...
                <div>
                    ...
                    <input type="number" name="sort_order" value="{{ old('sort_order', $service->sort_order ?? 0) }}" min="0" class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-300 focus:border-teal-400">
                </div>
        ...
        <div class="flex items-center justify-end gap-3">
            <a href="{{ route('admin.services.index') }}" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">Cancel</a>
