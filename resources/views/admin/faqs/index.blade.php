...
@section('breadcrumb')
    <nav class="flex items-center gap-1 text-xs text-gray-500">
        ...
        <span>/</span>
        <span class="text-gray-700 font-medium">FAQs</span>
    </nav>
@endsection
    ...
    <div class="p-4 sm:p-5 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <form method="GET" class="flex flex-wrap gap-3 flex-1">
            <div class="relative flex-1 min-w-[180px]">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search FAQs..." class="w-full pl-9 pr-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-300 focus:border-teal-400">
            </div>
            ...
            @if(request()->anyFilled(['search', 'is_active']))
                <a href="{{ route('admin.faqs.index') }}" class="px-3 py-2 text-sm font-medium text-gray-600 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">Clear</a>
            @endif
        </form>
        <div class="flex items-center gap-2">
            <form action="{{ route('admin.faqs.activate-all') }}" method="POST" class="inline">
                @csrf
                <button type="submit" class="px-3 py-2 text-sm font-medium text-teal-600 bg-teal-50 border border-teal-200 rounded-lg hover:bg-teal-100 transition-colors" onclick="return confirm('Activate all FAQs?')">Activate All</button>
            </form>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-5 py-3 font-medium text-gray-900 max-w-xs truncate">{{ $faq->question }}</td>
                        <td class="px-5 py-3 text-gray-600 max-w-sm truncate hidden sm:table-cell">{{ Str::limit($faq->answer, 80) }}</td>
                        ...
                        <td class="px-5 py-3 text-center text-gray-500 hidden md:table-cell">{{ $faq->sort_order }}</td>
                        <td class="px-5 py-3 text-right">
                            <div class="flex items-center justify-end gap-1">
                                <a href="{{ route('admin.faqs.edit', $faq) }}" class="p-1.5 text-gray-400 hover:text-teal-600 hover:bg-teal-50 rounded-lg transition-colors">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10"/></svg>
                                </a>
                                <button type="button" class="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" @@click="$dispatch('open-confirm-delete', { url: '{{ route('admin.faqs.destroy', $faq) }}', method: 'DELETE' })">
