...
<div class="bg-white rounded-xl shadow-sm border border-gray-100">
    <div class="p-4 sm:p-5 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <form method="GET" class="flex gap-3 flex-1 max-w-md">
            <div class="relative flex-1">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search users..." class="w-full pl-9 pr-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-300 focus:border-teal-400">
            </div>
    ...
    @if($users->isEmpty())
        @include('admin.components.empty-state', [
            'title' => 'No users yet',
            'message' => 'Create your first admin user.',
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-5 py-3">
                            @include('admin.components.badge', [
                                'type' => $user->role === 'super_admin' ? 'danger' : ($user->role === 'admin' ? 'warning' : 'gray'),
                                'slot' => ucfirst(str_replace('_', ' ', $user->role))
                            ])
                        </td>
