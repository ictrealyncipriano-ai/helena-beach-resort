...
<form method="POST" action="{{ $cottage->exists ? route('admin.cottages.update', $cottage) : route('admin.cottages.store') }}" enctype="multipart/form-data" class="space-y-6" x-data="cottageForm()">
        ...
        <div class="p-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mt-5">
                <div>
                    ...
                    <input type="number" name="capacity" value="{{ old('capacity', $cottage->capacity) }}" min="0" class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-600 focus:border-teal-400 dark:bg-slate-800 dark:border-slate-600 dark:text-white dark:placeholder-slate-400 dark:focus:border-teal-500 dark:focus:ring-teal-500/20">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1 dark:text-slate-300">Day Tour Rate (₱)</label>
                    <input type="number" step="0.01" name="rate_daytour" value="{{ old('rate_daytour', $cottage->rate_daytour) }}" min="0" class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-600 focus:border-teal-400 dark:bg-slate-800 dark:border-slate-600 dark:text-white dark:placeholder-slate-400 dark:focus:border-teal-500 dark:focus:ring-teal-500/20">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1 dark:text-slate-300">Overnight Rate (₱)</label>
                    <input type="number" step="0.01" name="rate_overnight" value="{{ old('rate_overnight', $cottage->rate_overnight) }}" min="0" class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-600 focus:border-teal-400 dark:bg-slate-800 dark:border-slate-600 dark:text-white dark:placeholder-slate-400 dark:focus:border-teal-500 dark:focus:ring-teal-500/20">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1 dark:text-slate-300">Sort Order</label>
                    <input type="number" name="sort_order" value="{{ old('sort_order', $cottage->sort_order ?? 0) }}" min="0" class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-600 focus:border-teal-400 dark:bg-slate-800 dark:border-slate-600 dark:text-white dark:placeholder-slate-400 dark:focus:border-teal-500 dark:focus:ring-teal-500/20">
                </div>
                <div class="flex items-end pb-2">
                    <label class="relative inline-flex items-center cursor-pointer">
                        ...
                        <div class="w-9 h-5 bg-gray-200 peer-checked:bg-teal-600 dark:bg-slate-600 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-teal-600 rounded-full peer peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all"></div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 dark:bg-slate-800 dark:border-slate-700">
        <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between dark:border-slate-700">
            ...
            <button type="button" @@click="addAmenity()" class="text-sm font-medium text-teal-700 hover:text-teal-700 flex items-center gap-1">
        ...
        <div class="p-5 space-y-3" id="amenities-container">
            <template x-for="(amenity, i) in amenities" :key="i">
                <div class="flex items-center gap-3">
                    <input type="text" :name="'amenities['+i+'][name]'" x-model="amenity.name" placeholder="Amenity name" class="flex-1 px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-600 focus:border-teal-400 dark:bg-slate-800 dark:border-slate-600 dark:text-white dark:placeholder-slate-400 dark:focus:border-teal-500 dark:focus:ring-teal-500/20">
                    <select :name="'amenities['+i+'][icon]'" x-model="amenity.icon" class="px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-600 focus:border-teal-400 bg-white dark:bg-slate-800 dark:border-slate-600 dark:text-white dark:focus:border-teal-500 dark:focus:ring-teal-500/20">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 dark:bg-slate-800 dark:border-slate-700">
        <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between dark:border-slate-700">
            ...
            <button type="button" @@click="addDateBlock()" class="text-sm font-medium text-teal-700 hover:text-teal-700 flex items-center gap-1">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.5v15m7.5-7.5h-15"/></svg>
                Add
            </button>
        </div>
        <div class="p-5 space-y-3">
            <template x-for="(block, i) in dateBlocks" :key="i">
                <div class="flex items-center gap-3">
                    <input type="date" :name="'date_blocks['+i+'][date]'" x-model="block.date" class="flex-1 px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-600 focus:border-teal-400 dark:bg-slate-800 dark:border-slate-600 dark:text-white dark:placeholder-slate-400 dark:focus:border-teal-500 dark:focus:ring-teal-500/20">
                    <input type="text" :name="'date_blocks['+i+'][reason]'" x-model="block.reason" placeholder="Reason (optional)" class="flex-1 px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-600 focus:border-teal-400 dark:bg-slate-800 dark:border-slate-600 dark:text-white dark:placeholder-slate-400 dark:focus:border-teal-500 dark:focus:ring-teal-500/20">
...
function cottageForm() {
    return {
        ...
        dateBlocks: @js(old('date_blocks', $cottage->exists ? $cottage->dateBlocks->filter(fn($b) => !str_contains((string) $b->reason, 'HB-'))->map(fn($b) => ['date' => $b->date?->format('Y-m-d'), 'reason' => $b->reason])->values()->toArray() : [])),
