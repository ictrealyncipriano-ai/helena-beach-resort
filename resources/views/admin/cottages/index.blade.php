...
@section('breadcrumb')
    <nav class="flex items-center gap-1 text-xs text-gray-500">
        ...
        <span>/</span>
        <span class="text-gray-700 font-medium">Cottages</span>
    </nav>
@endsection
    ...
    <div class="p-4 sm:p-5 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <form method="GET" class="flex flex-col sm:flex-row gap-3 flex-1 max-w-lg">
            <div class="relative flex-1">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search cottages..." class="w-full pl-9 pr-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-300 focus:border-teal-400 transition-all">
            </div>
            <select name="availability" class="px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-300 focus:border-teal-400 bg-white">
            ...
            @if(request()->anyFilled(['search', 'availability']))
                <a href="{{ route('admin.cottages.index') }}" class="px-3 py-2 text-sm font-medium text-gray-600 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors text-center">Clear</a>
            @endif
        </form>
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-5 py-3">
                            <div class="flex items-center gap-3">
                                ...
                                @endif
                                <span class="font-medium text-gray-900">{{ $cottage->name }}</span>
                            </div>
                        </td>
                        <td class="px-5 py-3 text-center text-gray-600">{{ $cottage->capacity }}</td>
                        <td class="px-5 py-3 text-right text-gray-600">₱ {{ number_format($cottage->rate_daytour, 2) }}</td>
                        <td class="px-5 py-3 text-right text-gray-600">₱ {{ number_format($cottage->rate_overnight, 2) }}</td>
                        ...
                        <td class="px-5 py-3 text-center text-gray-500">{{ $cottage->sort_order }}</td>
                        <td class="px-5 py-3 text-right">
                            <div class="flex items-center justify-end gap-1">
                                <a href="{{ route('admin.cottages.edit', $cottage) }}" class="p-1.5 text-gray-400 hover:text-teal-600 hover:bg-teal-50 rounded-lg transition-colors" title="Edit">
