...
@section('breadcrumb')
    <x-admin.breadcrumb :items="[['label' => 'Site Settings']]" />
@endsection

@section('content')
<div x-data="siteSettingModal()" class="space-y-6">
    <div class="bg-teal-50 border border-teal-200 rounded-xl px-5 py-4 text-sm text-teal-800 leading-relaxed dark:bg-teal-900/30 dark:border-teal-800 dark:text-teal-200">
        <p class="font-semibold mb-1">Second-resort keys</p>
        <p><span class="font-mono text-xs">site_logo</span> / <span class="font-mono text-xs">site_favicon</span> take a Storage path (e.g. <span class="font-mono text-xs">logos/sunset.png</span>); empty falls back to shipped files. <span class="font-mono text-xs">theme_color</span> is a hex code. <span class="font-mono text-xs">geo_region</span>, <span class="font-mono text-xs">geo_placename</span> and <span class="font-mono text-xs">address_locality|region|country</span> drive maps and structured data. <span class="font-mono text-xs">booking_cutoff_hours</span> (1–168, default 24) and <span class="font-mono text-xs">booking_hold_hours</span> (1–168, default 48) govern modification windows and pending-request expiry; invalid values safely fall back. <span class="font-mono text-xs">cancellation_policy_json</span> sets tiered refunds of collected money (default 168h/100, 72h/50, 24h/0); invalid JSON safely falls back. <span class="font-mono text-xs">about_body</span> (sanitized HTML), <span class="font-mono text-xs">footer_tagline</span> and <span class="font-mono text-xs">operating_hours</span> drive the About page, footer, and contact info. <span class="font-mono text-xs">og_image</span> sets the social share image (empty falls back to the logo).</p>
    </div>
    ...
    <x-admin.modal name="site-setting-form" size="lg">
        <form method="POST" :action="formAction">
            ...
            <div class="flex items-center justify-end gap-3 pt-5 mt-6 border-t border-gray-100 dark:border-slate-700">
                <button type="button" @@click="$dispatch('close-modal-site-setting-form')" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors dark:bg-slate-800 dark:border-slate-600 dark:text-slate-200 dark:hover:bg-slate-700">Cancel</button>
...
<x-admin.confirm-dialog name="delete" title="Delete Setting?" message="Are you sure? This cannot be undone." />
@endsection
