...
@section('breadcrumb')
    <x-admin.breadcrumb :items="[['label' => 'Testimonials']]" />
@endsection
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-50 dark:divide-slate-700/50">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors dark:hover:bg-slate-700/40">
                            ...
                            <td class="px-5 py-3 hidden sm:table-cell"><x-admin.badge type="primary" :label="$testimonial->cottage?->name ?? 'N/A'" /></td>
                            ...
                            <td class="px-5 py-3 text-center">
                                @if($testimonial->source === 'guest')
                                    <x-admin.badge type="warning" label="Guest" />
                                @else
                                    <x-admin.badge type="gray" label="Admin" />
                                @endif
                            </td>
    ...
    <x-admin.modal name="testimonial-form" size="lg">
        <form method="POST" :action="formAction" enctype="multipart/form-data" @@submit="attachAvatar($event)">
            ...
            <div class="flex items-center justify-end gap-3 pt-5 mt-6 border-t border-gray-100 dark:border-slate-700">
                <button type="button" @@click="$dispatch('close-modal-testimonial-form')" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors dark:bg-slate-800 dark:border-slate-600 dark:text-slate-200 dark:hover:bg-slate-700">Cancel</button>
...
<x-admin.confirm-dialog name="delete" title="Delete Testimonial?" message="Are you sure? This cannot be undone." />
@endsection
